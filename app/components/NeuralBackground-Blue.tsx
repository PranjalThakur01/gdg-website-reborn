...
// Returns Electric Blue for dark mode and Deep Navy for light mode
function getPrimaryColor(): string {
  if (typeof window === "undefined") return "59, 130, 246" // Default Blue

  const isDark = document.documentElement.classList.contains("dark")
  // Dark mode: Electric Blue (RGB) - Bright and glowing
  // Light mode: Deep Navy (RGB) - Dark and legible
  return isDark ? "0, 190, 255" : "0, 40, 120"
}
...
function getBlendMode(): string {
  ...
  const isDark = document.documentElement.classList.contains("dark")
  // 'screen' makes the blue glow on dark backgrounds
  // 'multiply' darkens the navy blue on light backgrounds
  return isDark ? "screen" : "multiply"
}

// Helper function to get opacity
function getOpacity(): number {
  return 0.9 // High opacity for better visibility
}
